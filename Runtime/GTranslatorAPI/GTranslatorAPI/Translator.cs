...
namespace GTranslatorAPI
{
    ...
    public class Translator : ITranslator
    {
        ...
        public async Task<Translation[]> TranslateCombineAsync(Languages sourceLanguage,
            Languages targetLanguage,
            string[] text)
        {
            return await RunTranslateCombineQueryAsync(sourceLanguage, targetLanguage, text);
        }
...
#pragma warning restore CS8604
                        ...
                        var jsonData = JsonConvert.DeserializeObject <List<object>>(r.Item1);          
                        var translationItems = jsonData[0] as IEnumerable;            
                        string translation = string.Empty;
                        string original = string.Empty;
                        foreach (object item in translationItems)
                        {            
                            var jsonSubData = JsonConvert.DeserializeObject <List<object>>(item.ToString());
                            translation += string.Format("{0}", jsonSubData[0]);
                            original += string.Format("{0}", jsonSubData[1]);
                        }
        ...
        async Task<Translation[]> RunTranslateCombineQueryAsync(
            Languages sourceLanguage,
            Languages targetLanguage,
            string[] text
        )
        {
            var q = BuildServiceUriPathAndQuery(string.Join("\n", text), sourceLanguage, targetLanguage);
            var r = await _net.GetQueryResponseAsync(q);
            var jsonData = JsonConvert.DeserializeObject <List<object>>(r.Item1);          
            var translationItems = jsonData[0] as IEnumerable;
            List<Translation> translationList = new List<Translation>();
            translationList = text.Select(x =>
            {
                ...
                {
                    var jsonSubData = JsonConvert.DeserializeObject <List<object>>(item.ToString());
                    var OriginalText = jsonSubData[1].ToString();
                    var TranslatedText = jsonSubData[0].ToString();
                    
                    if (x.Contains(OriginalText))
                    {
                        translation.OriginalText += OriginalText;
                        translation.TranslatedText += TranslatedText;
                    }

                    if (translation.OriginalText == x)
                    {
                        break;
                    }
                }
